...
var files = mysqlTable("files", {
  ...
  // Categoria: general, portfolio, etc
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull()
});
var projects = mysqlTable("projects", {
  ...
  // eletrica, marcenaria, pintura
  status: mysqlEnum("status", ["draft", "published", "archived"]).default("draft"),
  featuredImageFileId: int("featuredImageFileId"),
  // Referência a um arquivo como imagem destaque
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull()
});
var projectFiles = mysqlTable("projectFiles", {
  id: int("id").autoincrement().primaryKey(),
...
var filesRouter = router({
  ...
  upload: protectedProcedure.input(
    z2.object({
      ...
      fileData: z2.instanceof(Buffer),
      mimeType: z2.string(),
      description: z2.string().optional(),
      category: z2.enum(["general", "portfolio", "other"]).default("general")
    })
  ).mutation(async ({ input, ctx }) => {
    try {
      const timestamp2 = Date.now();
      const randomSuffix = Math.random().toString(36).substring(7);
      const fileKey = `files/${ctx.user.id}/${timestamp2}-${randomSuffix}-${input.fileName}`;
  ...
  getById: protectedProcedure.input(z2.object({ fileId: z2.number() })).query(async ({ input, ctx }) => {
    try {
      ...
      if (file.userId !== ctx.user.id) {
        throw new TRPCError3({
          code: "FORBIDDEN",
          message: "Voc\xEA n\xE3o tem permiss\xE3o para acessar este arquivo"
        });
      }
      return file;
    } catch (error) {
  ...
  // Deletar arquivo
  delete: protectedProcedure.input(z2.object({ fileId: z2.number() })).mutation(async ({ input, ctx }) => {
    try {
      const file = await getFileById(input.fileId);
      if (!file) {
        throw new TRPCError3({
      ...
      if (file.userId !== ctx.user.id) {
        throw new TRPCError3({
          code: "FORBIDDEN",
          message: "Voc\xEA n\xE3o tem permiss\xE3o para deletar este arquivo"
        });
      }
  ...
  getPublicUrl: protectedProcedure.input(z2.object({ fileId: z2.number() })).query(async ({ input, ctx }) => {
    try {
      const file = await getFileById(input.fileId);
      if (!file) {
        throw new TRPCError3({
